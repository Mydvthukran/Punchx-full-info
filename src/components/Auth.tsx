  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (result && result.user) {
        ...
        const userName = result.user.displayName || (userEmail ? userEmail.split('@')[0] : 'User');
        const userUid = result.user.uid;

        if (activePanelRole === 'admin' && userEmail.toLowerCase() !== ADMIN_DASHBOARD_EMAIL.toLowerCase()) {
          showNotification(`⚠️ Unauthorized. Only ${ADMIN_DASHBOARD_EMAIL} is authorized for dashboard access.`);
      ...
      showNotification("⚠️ Google Sign-In was cancelled or unavailable. Please try again or use password login.");
    } finally {
      setIsSubmitting(false);
    }
  };
